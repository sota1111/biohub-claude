"""SOT-2993 — leak-free CV A/B: masked-sparse learned 3D U-Net detector vs the
classical DoG+NMS champion (and vs the prior naive/cellsparse learned arms).

Detection-only swap, linking fixed (champion linker): every arm runs the SAME
leak-free harness (``biohub_tracking.eval.cv`` primitives), differing ONLY in the
per-voxel detector, so the A/B isolates the detector. Arms:

* ``champion``   — classical DoG+NMS via the harness ``evaluate_cv()`` (KPI ref).
* ``masked``     — SOT-2993 masked-sparse-loss weights (this issue).
* ``naive``      — SOT-2848 foreground-weighted-MSE weights (optional reference).
* ``cellsparse`` — SOT-2863 Cellsparse ignore-weighting weights (optional reference).

Promotion gate (CLAUDE.md / SOT-2817): a learned arm is promoted only if it beats
the champion micro-adj beyond the noise band AND regresses no single family;
otherwise the champion stays byte-frozen and the arm ships default-off. Runs in the
repo ``.venv`` (torch + scipy + zarr).
"""
...
ARM_WEIGHT_DIRS = {
    "masked": REPO / "experiments/sot2993/weights/masked",
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--arms", default="masked",
    ...
    ap.add_argument("--out", default=str(REPO / "experiments/sot2993/screen_masked_ab.json"))
